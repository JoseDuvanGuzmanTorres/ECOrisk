...
public interface erRiesgos_ValoracionRepository extends JpaRepository<erRiesgos_Valoracion, Integer> {
    ...
    @Query(value="SELECT SUM(CASE WHEN e.ra_rv_id = 1 AND en.er_proceso_id = 1 THEN 1 ELSE 0 END) as InicialCapexN" 
        ...
        + "			,SUM(CASE WHEN e.rr_rv_id = 1 AND en.er_proceso_id = 1 AND e.er_estado_id = 1 THEN 1 ELSE 0 END) as AbiertosN"
        + "			,SUM(CASE WHEN e.rr_rv_id = 2 AND en.er_proceso_id = 1 AND e.er_estado_id = 1 THEN 1 ELSE 0 END) as AbiertosL"
        + "			,SUM(CASE WHEN e.rr_rv_id = 3 AND en.er_proceso_id = 1 AND e.er_estado_id = 1 THEN 1 ELSE 0 END) as AbiertosM"
        + "			,SUM(CASE WHEN e.rr_rv_id = 4 AND en.er_proceso_id = 1 AND e.er_estado_id = 1 THEN 1 ELSE 0 END) as AbiertosH"
        + "			,SUM(CASE WHEN e.rr_rv_id = 5 AND en.er_proceso_id = 1 AND e.er_estado_id = 1 THEN 1 ELSE 0 END) as AbiertosVH"
