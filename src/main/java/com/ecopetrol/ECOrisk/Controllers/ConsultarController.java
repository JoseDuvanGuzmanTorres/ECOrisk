...
public class ConsultarController {
	...
	//encabezadosList = ErEncabezadoService.getProjectionByUser(null, null, null, null, null, null, null, null, null, null, null, null, Finicial, Ffinal,users.getId());

	@GetMapping("projection/encabezado")
	@ResponseBody
	public List<erEncabezadoProjection> findByTrabajoId(@RequestParam(required = false) Integer user_id,@RequestParam(required = false) String proyecto) {
		...
		List<erEncabezadoProjection> encabezadosList = ErEncabezadoService.getProjectionByUser(tipoestudio, proceso , proyecto, codigo, nombre, objetivo, etapa , regional, gerencia, dueno, departamento, lider, instalacion, Finicial, Ffinal, user_id);
	...
	public List<erEncabezadoProjection> findByDueId(@RequestParam Integer dueno) {
		...
		List<erEncabezadoProjection> encabezadosList = ErEncabezadoService.getProjectionByUser(tipoestudio, proceso , proyecto, codigo, nombre, objetivo, etapa , regional, gerencia, duenio, departamento, lider, instalacion, Finicial, Ffinal, user_id);
	...
	public String loadWhatIf(Model model, 
			@RequestParam(required = false) String tipoestudio, @RequestParam(required = false) String proceso,
			@RequestParam(required = false) String proyecto, @RequestParam(required = false) String codigo,
			@RequestParam(required = false) String nombre, @RequestParam(required = false) String objetivo,
			@RequestParam(required = false) String dueno, @RequestParam(required = false) String etapa,
			@RequestParam(required = false) String regional, @RequestParam(required = false) String gerencia,
			@RequestParam(required = false) String departamento, @RequestParam(required = false) String lider,
			@RequestParam(required = false) String instalacion, @RequestParam(required = false) String fechainicial,
			@RequestParam(required = false) String fechafinal, Principal principal) {
		...
		if(users.getRoles_id() == 1 || users.getRoles_id() == 2 || users.getRoles_id() == 3 || users.getRoles_id() == 6 || users.getRoles_id() == 7) {
			Id = null;
		}else {
			Id = users.getId();
		}
		
		List<erEncabezadoProjection> encabezadosList = ErEncabezadoService.getProjectionByUser(tipoestudio, proceso ,proyecto, codigo, nombre, objetivo, etapa , regional, gerencia, dueno, departamento, lider, instalacion, Finicial, Ffinal, Id);
		...
		List<er_HojaTrabajoProjection> MasterhojatrabajoList2 = Er_HojaTrabajoService.getAllProjectionByIdEncabeAndUserId(id_encabeList,tipoestudio, Id);
		model.addAttribute("MasterhojatrabajoList", MasterhojatrabajoList2);
		
		List<er_HojaTrabajoLeccionesAProjection> LeccioneshojatrabajoList = Er_HojaTrabajoService.getAllHojaTrabajoLAProjectionByEncabezado(id_encabeList,Id);
		model.addAttribute("LeccioneshojatrabajoList", LeccioneshojatrabajoList);
		
		List<er_HojaTrabajoREmergenteProjection> REhojatrabajoList = Er_HojaTrabajoService.getAllHojaTrabajoREProjectionByEncabezado(id_encabeList,Id);
		model.addAttribute("REhojatrabajoList", REhojatrabajoList);
		
		List<er_HojaTrabajoRMaterializadoProjection> RMhojatrabajoList = Er_HojaTrabajoService.getAllHojaTrabajoRMProjectionByEncabezado(id_encabeList,Id);
